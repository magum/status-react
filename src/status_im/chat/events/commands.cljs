...
(defn generate-context
  ...
  [{:keys [chats handler-data] :accounts/keys [current-account-id]}
   message-id chat-id to group-id message-handler-data]
  (merge {:platform     platform/platform
          ...
          :chat         {:chat-id    chat-id
                         :group-chat (or (get-in chats [chat-id :group-chat])
                                         (not (nil? group-id)))}
          :handler-data (merge message-handler-data (get handler-data message-id {}))}
         i18n/delimeters))
      ...
      (if (get-in contacts [jail-id :commands-loaded?])
        (let [path          [(if (= :response (keyword type)) :responses :commands)
              ...
              jail-params   {:parameters params
                             :context (generate-context db message-id chat-id to group-id handler-data)}]
          (when (= :preview data-type)
            (log/debug (str "Preview context handler-data : " (-> jail-params :context :handler-data))))
